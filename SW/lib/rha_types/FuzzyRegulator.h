...
class FuzzyRegulator {
    ...
    float regulator(float _actual_point, float _error, float _derror = 0, float _ierror = 0) {
        float result = 0;
        for (uint8_t i = 0; i < NUM_FUZZY_REGULATOR_NODES; i++) {
            // The evaluation is made between the intervals, thats why this check is made, so it never reaches to evaluate nodes[NUM_FUZZY_REGULATOR_NODES+1], which will cause a segfault
            if (i > (NUM_FUZZY_REGULATOR_NODES - 1)) break;
