...
void test_function_warpSinglePacket(void) {
    ...
    uint8_t buffer_test1[4] = { 0xFF, 0xFF, SERVO_ID, 0x04, iWRITE_DATA, ServoRHAConstants::TORQUE_ENABLE, ON};

    jh_test1.joint_[i].servo_.setTorqueOnOfToPacket(buffer, ON);
...
void test_function_warpSyncPacket(void) {
    ...
    jh_test1.warpSyncPacket(iWRITE_DATA, buffer_to_send, txBuffer, num_bytes, num_servo);
    ...
    TEST_ASSERT_EQUAL_UINT8(28, txBuffer[4]+4);
    TEST_ASSERT_EQUAL_UINT8_ARRAY(buffer_test, txBuffer, txBuffer[4]+4);
}
